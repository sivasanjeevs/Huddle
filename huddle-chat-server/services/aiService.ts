class AIService {
   ...
   */
  async answerEventQuestions(question, lobbyId) {
    return this._postToEngine('/answer-event-questions', { question, lobby_id: lobbyId });
  }

  /**
   * Find turfs near a specific location
   */
  async findTurfs(query, lobbyId) {
    return this._postToEngine('/find-turfs', { query, lobby_id: lobbyId });
  }
}
