...
export default function LobbyCard({
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: lobby.title, text: `Check out ${lobby.title} on Huddle!`, url });
      } catch (err) {
  ...
  const handleJoinClick = async () => {
    if (isJoined) return;
    setJustJoined(true);
    await handleJoin(lobby.id);
    setTimeout(() => setJustJoined(false), 1200);
  };

  return (
    <div className="shimmer-hover group bg-white/70 backdrop-blur-xl border border-white/80 shadow-[0_4px_24px_rgb(0,0,0,0.05)] hover:shadow-[0_8px_40px_rgb(0,0,0,0.12)] hover:bg-white/95 hover:border-blue-100 rounded-3xl overflow-hidden flex transition-all duration-300 mb-6">
      {/* Main Content */}
      <div className="flex-1 p-4 sm:p-6 flex flex-col min-w-0">
        ...
        <div className="flex flex-wrap items-center gap-2 mb-2 text-xs">
          ...
          <span className="text-slate-500">Posted by u/{lobby.creator?.name || 'unknown'}</span>
          <span className="text-slate-500">•</span>
          ...
          <span className="text-slate-500 flex items-center gap-1 ml-auto">
            <span className="relative flex h-2 w-2">
              {lobby.active && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75" />}
              <span className={`relative inline-flex rounded-full h-2 w-2 ${lobby.active ? 'bg-green-500' : 'bg-slate-300'}`} />
            </span>
        ...
        <div className="mt-auto flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2 mt-2 sm:mt-0 w-full sm:w-auto">
            ...
            {/* ── JOIN BUTTON with micro-animation ── */}
            <button
              onClick={handleJoinClick}
              disabled={isJoined}
              className={`relative px-5 py-2 text-sm font-semibold rounded-xl transition-all shadow-sm overflow-hidden
                ${justJoined ? 'animate-join-pulse bg-emerald-500 text-white' : ''}
                ${isJoined && !justJoined ? 'bg-slate-100 text-slate-400 cursor-default shadow-none' : ''}
                ${!isJoined && !justJoined ? 'bg-slate-900 text-white hover:bg-slate-800 hover:shadow-md' : ''}
              `}
            >
              {justJoined ? (
